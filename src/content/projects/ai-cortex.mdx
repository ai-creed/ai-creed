...
> v0.10.3 — actively used. Available on npm.
...
agent → rehydrate_project
  ai-cortex · v0.10.3 · 330 files · pinned: "never write into the target repo"
...
Recall depends on the agent *remembering to ask*. Surface doesn't. Before an edit lands, ai-cortex slips the file's relevant memories into the agent's working context — the rule shows up at the moment it matters, not after the mistake. Available in Claude Code today; Codex follows as the harness opens the same hook.
