...
features:
  - "structured workflows — spec-driven-development, ralph-loop, complex-bug-fixing"
...
> v0.3.0 — open source, on npm, actively used personally.

## what it does

ai-whisper pairs two coding agents — Claude and Codex today, provider-agnostic by design — in your terminal. They share a single baton — one owns the turn at a time — and structured workflows drive the collaboration, with an evaluator gating each round. The relay isn't tied to any one arrangement; today's workflows pair an implementer and a reviewer (spec-driven-development, ralph-loop, complex-bug-fixing).
